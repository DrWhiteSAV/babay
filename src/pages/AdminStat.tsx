...
interface DailyActivity { day: string; messages: number; }
...
export default function AdminStat() {
  ...
  const loadStats = async () => {
    setLoading(true);
    const [pCount, sData, chatCount, galCount, achCount, msgs, topRes] = await Promise.all([
      ...
      supabase.from("player_stats").select("character_name, fear, telegram_id")
        .order("fear", { ascending: false }).limit(5),
    ]);
    ...
    for (let i = 6; i >= 0; i--) {
      const key = new Date(Date.now() - i * 86400000).toLocaleDateString("ru-RU", { weekday: "short" });
      days[key] = 0;
    }
    (msgs.data || []).forEach(m => {
      const key = new Date(m.created_at).toLocaleDateString("ru-RU", { weekday: "short" });
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-5">
        <div className="flex items-center justify-between">
          ...
          <button
            onClick={loadStats}
            disabled={loading}
            className="flex items-center gap-1.5 text-xs text-neutral-400 hover:text-white transition-colors px-3 py-1.5 rounded-lg bg-neutral-900 border border-neutral-800"
          >
          ...
          <>
            ...
            <div className="bg-neutral-900 border border-neutral-800 rounded-2xl p-4">
              ...
              <div className="flex items-end gap-1.5 h-28 justify-between">
                {daily.map((d, i) => {
                  const pct = (d.messages / maxDaily) * 100;
                  return (
                    <div key={i} className="flex-1 flex flex-col items-center gap-1">
                      <span className="text-[9px] text-neutral-600 font-mono">{d.messages || ""}</span>
                      <div className="w-full bg-neutral-800 rounded-t-sm relative" style={{ height: "64px" }}>
                        <motion.div
            ...
            {topPlayers.length > 0 && (
              <div className="bg-neutral-900 border border-neutral-800 rounded-2xl overflow-hidden">
                <h3 className="text-sm font-bold text-white p-4 border-b border-neutral-800 flex items-center gap-2">
                  <Zap size={16} className="text-red-500" /> Топ-5 по Страху
                </h3>
                {topPlayers.map((p, i) => (
                  <div key={p.telegram_id} className="flex items-center gap-3 px-4 py-2.5 border-b border-neutral-800/50 last:border-0">
                    ...
                    <span className="flex-1 text-sm text-white truncate">{p.character_name || `ID:${p.telegram_id}`}</span>
